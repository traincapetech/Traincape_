...
import { FaHome, FaCertificate, FaBook, FaAws } from 'react-icons/fa'; // Icons
...
// Main Tab Container with Sidebar
const TabContainer = ({ tabs }) => {
  ...
  return (
    <div className="grid sm:grid-flow-row grid-cols-1 lg:flex">
      {/* Sidebar */}
      <nav className={`bg-gray-800 text-white lg:w-64 w-full p-4 flex flex-col space-y-4 lg:block ${isSidebarOpen ? 'block' : ''}`}>
        {Object.keys(tabs).map((key) => (
          <NavTab
            ...
            icon={
              key === 'Overview' ? <FaHome className='w-7 h-7' />
                : key === 'Certify' ? <FaCertificate className='w-7 h-7' />
                : key === 'Learn' ? <FaBook className='w-7 h-7' />
                : key === 'Contact' ? <FaPhoneVolume className='w-7 h-7' />
                : key === 'Practice' ? <BsFillPenFill className='w-7 h-7' />
