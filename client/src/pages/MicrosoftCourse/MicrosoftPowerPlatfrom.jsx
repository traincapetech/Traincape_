...
const NavTab = ({ label, active, onClick, icon }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`text-lg flex w-full gap-4 items-center hover:bg-gray-700 px-4 py-2 rounded transition ${
        active ? "bg-gray-700" : ""
...
const TabContainer = ({ tabs }) => {
  ...
  return (
    <div className="grid sm:grid-flow-row grid-cols-1 lg:flex">
      {/* Sidebar */}
      <nav
        className={` bg-gray-800 text-white lg:w-64 w-full p-4 flex flex-col space-y-4 lg:block ${
          isSidebarOpen ? "block" : ""
        } `}
      >
...
const MicrosoftPowerPlatfrom = () => {
  const tabs = {
    Overview: <MicrosoftOverview title={"Microsoft Power Platfrom"} />,
    Certify: <Certify title={"Microsoft Power Platfrom"} />,
    Learn: <Learn title={"Microsoft Power Platfrom "} />,
    Practice: (
      <Practice course={"Microsoft"} subTopic={"MicrosoftPowerPlatform"} />
    ),
    "My Pathway": <MyPathway title={"Microsoft Power Platfrom"} />,
    // Resources:< Resources/>,
    Contact: <ContactUs />,
  };
...
export default MicrosoftPowerPlatfrom;
