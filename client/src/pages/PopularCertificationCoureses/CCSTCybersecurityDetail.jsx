import React, { useEffect, useState } from "react";
import { IoMdArrowRoundBack } from "react-icons/io";
import { IoIosArrowDown, IoIosArrowUp } from "react-icons/io";
import { Link } from "react-router-dom";

const CCSTCybersecurityDetail = () => {
  const [selectedQuestion, setSelectedQuestion] = useState(null);

  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  const toggleAnswer = (id) => {
    setSelectedQuestion(selectedQuestion === id ? null : id);
  };

  return (
    <div className="bg-[#F9F9F9] overflow-hidden">
      <div className="bg-gradient-to-r from-gray-800 to-gray-600 py-20 px-4">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between">
          <div className="md:w-1/2 text-white mb-8 md:mb-0">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">CCST – Cybersecurity</h1>
            <p className="text-xl md:text-2xl mb-6">
              Build a solid foundation in cybersecurity concepts, tools, and practices to secure systems and protect data.
            </p>
            <div className="flex items-center space-x-4">
              <span className="bg-white text-gray-700 text-sm font-medium px-3 py-1 rounded-full">
                Cyber Defense
              </span>
              <span className="bg-white text-gray-700 text-sm font-medium px-3 py-1 rounded-full">
                Beginner Level
              </span>
            </div>
          </div>
          <div className="md:w-1/3">
            <img
              src={require("../../assets/Cybersecurity.jpg")}
              alt="Cybersecurity"
              className="w-full h-auto bg-white p-6 rounded-lg shadow-lg"
            />
          </div>
        </div>
      </div>

      <div className="max-w-6xl mx-auto px-4 py-8 md:py-16">
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="lg:w-2/3">
            <Link to="/Courses-details">
              <button className="flex items-center text-gray-700 mb-6 hover:text-black transition duration-300">
                <IoMdArrowRoundBack className="mr-2" />
                Back to Course Details
              </button>
            </Link>

            <div className="mb-8 border-b border-gray-200">
              <nav className="flex space-x-8">
                <button className="border-b-2 border-gray-700 text-gray-700 pb-4 font-medium">
                  Overview
                </button>
                <button className="text-gray-500 pb-4 font-medium">Curriculum</button>
                <button className="text-gray-500 pb-4 font-medium">Career Paths</button>
                <button className="text-gray-500 pb-4 font-medium">FAQ</button>
              </nav>
            </div>

            <div className="mb-12">
              <h2 className="text-2xl font-bold mb-4">Course Overview</h2>
              <p className="text-gray-700 mb-4">
                This course introduces the fundamentals of cybersecurity, including network security, system protection, and threat detection.
              </p>
              <p className="text-gray-700 mb-4">
                Ideal for students or professionals looking to build a foundation in cyber defense and risk management.
              </p>
            </div>

            <div className="mb-12">
              <h2 className="text-2xl font-bold mb-4">What You'll Learn</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[
                  "Cybersecurity principles & policies",
                  "Types of malware and attacks",
                  "Firewall & antivirus basics",
                  "Network and endpoint protection",
                  "Risk assessment & threat modeling",
                  "Incident response fundamentals",
                  "Access controls & authentication",
                  "Ethical hacking basics",
                ].map((item, i) => (
                  <div key={i} className="flex items-start">
                    <div className="h-5 w-5 bg-gray-700 rounded-full mt-1 flex-shrink-0" />
                    <p className="ml-3 text-gray-700">{item}</p>
                  </div>
                ))}
              </div>
            </div>

            <div className="mb-12">
              <h2 className="text-2xl font-bold mb-4">Career Opportunities</h2>
              <p className="text-gray-700 mb-4">
                Cybersecurity skills are in high demand across industries. This certification prepares you for entry-level roles in IT security.
              </p>
              <div className="bg-gray-50 p-6 rounded-lg">
                <h3 className="text-xl font-semibold mb-3">Job Roles & Average Salaries</h3>
                <ul className="space-y-2">
                  <li className="flex justify-between">
                    <span>Cybersecurity Analyst</span> <span>₹4–7 LPA</span>
                  </li>
                  <li className="flex justify-between">
                    <span>IT Security Support</span> <span>₹3–5.5 LPA</span>
                  </li>
                  <li className="flex justify-between">
                    <span>Network Security Technician</span> <span>₹4–6 LPA</span>
                  </li>
                </ul>
              </div>
            </div>

            <div className="mb-12">
              <h2 className="text-2xl font-bold mb-4">Exam Details</h2>
              <div className="bg-white p-6 rounded-lg shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h3 className="font-semibold text-gray-800 mb-2">Format</h3>
                  <ul className="text-gray-700 space-y-2">
                    <li>MCQs + scenario-based questions</li>
                    <li>Duration: 90 minutes</li>
                    <li>Questions: 50</li>
                    <li>Passing Score: 70%</li>
                  </ul>
                </div>
                <div>
                  <h3 className="font-semibold text-gray-800 mb-2">Topics</h3>
                  <ul className="text-gray-700 space-y-2">
                    <li>• Threats & vulnerabilities</li>
                    <li>• Network & system defense</li>
                    <li>• Security tools usage</li>
                    <li>• Cyber laws & policies</li>
                  </ul>
                </div>
              </div>
            </div>

            <div className="mb-12">
              <h2 className="text-2xl font-bold mb-6">FAQs</h2>
              {[
                {
                  q: "Is prior experience needed?",
                  a: "No prior experience is required, but basic computer knowledge is recommended.",
                },
                {
                  q: "Is this course suitable for college students?",
                  a: "Yes, it's perfect for students preparing for careers in cybersecurity.",
                },
                {
                  q: "Will I get certification after completion?",
                  a: "Yes, after passing the exam, you'll receive an industry-recognized certificate.",
                },
              ].map(({ q, a }, i) => (
                <div key={i} className="border border-gray-200 rounded-lg overflow-hidden mb-4">
                  <button
                    onClick={() => toggleAnswer(i)}
                    className="flex justify-between items-center w-full p-4 text-left bg-white hover:bg-gray-50"
                  >
                    <span className="font-medium">{q}</span>
                    {selectedQuestion === i ? (
                      <IoIosArrowUp className="text-gray-700" />
                    ) : (
                      <IoIosArrowDown className="text-gray-700" />
                    )}
                  </button>
                  {selectedQuestion === i && (
                    <div className="p-4 bg-gray-50 border-t border-gray-200">
                      <p className="text-gray-700">{a}</p>
                    </div>
                  )}
                </div>
              ))}
            </div>
          </div>

          <div className="lg:w-1/3">
            <div className="bg-white rounded-xl shadow-lg p-6 sticky top-8">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <p className="text-2xl font-bold">₹1,499</p>
                  <p className="text-gray-500 line-through">₹1,999</p>
                </div>
                <span className="bg-red-100 text-red-600 text-xs font-medium px-2.5 py-0.5 rounded-full">
                  25% OFF
                </span>
              </div>
              <p className="text-sm text-gray-500 mb-6">
                Secure your career with certified cybersecurity training.
              </p>
              <div className="mt-6">
                <h3 className="font-semibold text-lg mb-3">Includes:</h3>
                <ul className="space-y-2 text-gray-700">
                  <li>✔ Beginner-friendly course</li>
                  <li>✔ Real-world scenarios</li>
                  <li>✔ Certification exam prep</li>
                  <li>✔ Support & discussion forum</li>
                  <li>✔ Lifetime access</li>
                </ul>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default CCSTCybersecurityDetail;
