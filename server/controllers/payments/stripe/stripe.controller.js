...
// ✅ CREATE PAYMENT SESSION (Supports Guest Checkout)
const StripePayment = async (req, res) => {
  const { lineItems, success_url, cancel_url, email, productIds } = req.body;
  try {
    if (!stripe) {
    ...
    if (email) {
      try {
        ...
        console.warn("⚠️ Could not fetch user from DB:", err.message);
      }
    }

    if (!user) {
      console.warn(`⚠️ Proceeding with guest checkout (no user found for email: ${email || "N/A"})`);
    }

    // ✅ Validate voucher inventory only if productIds exist
    if (Array.isArray(productIds) && productIds.length > 0) {
      const requested = {};
      for (const pid of productIds) {
        if (typeof pid === 'string' && pid.startsWith('voucher-')) {
      ...
      for (const key of Object.keys(requested)) {
        const [course, subCourse] = key.split('__');
        const availableCount = await Voucher.countDocuments({ course, subCourse, status: 'available' });
        if (availableCount < requested[key]) {
          return res.status(400).json({ success: false, message: `Insufficient vouchers for ${course} ${subCourse}. Available ${availableCount}, requested ${requested[key]}` });
        }
      }
    ...
    const totalInCents = lineItems.reduce((sum, item) => sum + Math.round(item.price_data.unit_amount) * item.quantity, 0);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${success_url}?session_id={CHECKOUT_SESSION_ID}&email=${encodeURIComponent(email || "")}`,
      cancel_url: cancel_url,
      metadata: {
    ...
    // ✅ Save transaction only if user exists
    if (user) {
...
// ✅ SUCCESS HANDLER (Unchanged except still works for guests)
const StripePaymentSuccess = async (req, res) => {
  try {
    const { session_id, email } = req.query;
    console.log("Processing payment success:", { session_id, email });
    ...
    // If email provided, try updating user transaction
    if (email) {
      const user = await UserModel.findOne({ email });
      if (user) {
        const transaction = user.transactions.find(
          t => t.stripeSessionId === session_id || t.transactionId === session.metadata?.transactionId
        );

        if (transaction && transaction.status !== "completed") {
          transaction.status = "completed";
          transaction.completedAt = new Date();
          transaction.stripePaymentIntent = session.payment_intent?.id;
          transaction.metadata = session.metadata;
          await user.save();
        }
      }
    }
...
// ✅ WEBHOOK HANDLER remains same
const StripeWebhook = async (req, res) => {
  try {
    const sig = req.headers["stripe-signature"];
    const endpointSecret = process.env.STRIPE_WEBHOOK_SECRET;
    if (!endpointSecret) return res.status(500).send("Webhook secret not configured");
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (email && email !== "guest") {
        try {
          const user = await UserModel.findOne({ email });
          if (user) {
            const tx = user.transactions?.find(t => t.stripeSessionId === session.id);
            if (tx && tx.status !== "completed") {
              tx.status = "completed";
              tx.completedAt = new Date();
              tx.stripePaymentIntent = session.payment_intent;
              await user.save();
            }
          }
        } catch (_) {}
